...
export default function Dealers() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const filteredProducts = data.filter(product => product.product_type === productType);
      setProducts(filteredProducts);
      const initialToggles = filteredProducts.reduce((acc, product) => ({
        ...acc,
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`${API_BASE_URL}/api/products/${productType.toLowerCase().replace(/\s+/g, '_')}/${selectedProduct.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
